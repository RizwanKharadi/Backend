...
class SubscriptionBillingService {
  ...
  async createSubscriptionCheckout(user, { billingCycle, seatLimit }) {
    ...
    const razorpayPlanId = await this.getOrCreateRazorpayPlan(billingCycle);
    ...
    const rzSubscription = await this.razorpay.subscriptions.create(rzOptions);
  ...
  async syncFromRazorpay(user) {
    ...
    if (['active', 'authenticated'].includes(rz.status)) {
