...
async function pushVoucherToTallyWithResult(voucher, company, body, extraLedgerEntries = []) {
  ...
  try {
    ...
    const hasTallyProof =
      Boolean(importResult.tallyGuid) ||
      Boolean(importResult.alreadyExisted) ||
      Number(importResult.created) > 0 ||
      Number(importResult.altered) > 0;

    if (!hasTallyProof) {
      throw new Error(
        'Tally did not confirm the voucher import. Check item names, party ledger, and desktop agent logs.'
      );
    }
    ...
    return {
      ...
      voucherNumber:
        importResult.voucherNumber ||
        (importResult.tallyGuid ? voucher.voucherNumber : undefined)
    };
  } catch (pushError) {
