...
const ProfitLossLedgerVouchersScreen = () => {
  const { t } = useTranslation();
  const navigation = useNavigation<any>();
  const route = useRoute<any>();
  const { selectedCompany } = useCompany();
  const { ledgerName, periodKey, reportKind = 'profit_loss' } = (route.params || {}) as RouteParams;
  ...
  const renderRow = ({ item }: { item: ProfitLossVoucherRow }) => (
    ...
    >
      <View style={styles.rowLeft}>
        <Text style={styles.vchNo}>{item.voucherNumber}</Text>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerCenter}>
          ...
          <Text style={styles.headerSub}>
            {isBalanceSheet ? 'Vouchers (books from → as on date)' : 'Vouchers in period'}
          </Text>
        </View>
      </View>
...
export default ProfitLossLedgerVouchersScreen;
